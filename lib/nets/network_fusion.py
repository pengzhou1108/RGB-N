...
class Network(object):
  ...
  def _smooth_l1_loss(self, bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    ...
    return loss_box
  def _smooth_l1_dist(self, bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    sigma_2 = sigma ** 2
    box_diff = bbox_pred - bbox_targets
    in_box_diff = bbox_inside_weights * box_diff
    abs_in_box_diff = tf.abs(in_box_diff)
  ...
  def _add_losses(self, sigma_rpn=3.0):
    with tf.variable_scope('loss_' + self._tag) as scope:
      ...
      if cfg.TRAIN.HNM:
        ...
        top_k = tf.cast(tf.minimum(tf.shape(rpn_cross_entropy_n_neg)[0], 100), tf.int32)
        rpn_cross_entropy_n_neg, _ = tf.nn.top_k(rpn_cross_entropy_n_neg, k=top_k)
        rpn_cross_entropy = tf.reduce_sum(rpn_cross_entropy_n_neg) / (tf.reduce_sum(tf.cast(bg, tf.float32)) + 1.0) \
                                + tf.reduce_sum(rpn_cross_entropy_n_pos) / (tf.reduce_sum(tf.cast(fg, tf.float32)) + 1.0)
      else:
      ...
      if cfg.TRAIN.HNM:
        rpn_loss_box_n = self._smooth_l1_dist(rpn_bbox_pred, rpn_bbox_targets, rpn_bbox_inside_weights,
                                          rpn_bbox_outside_weights, sigma=sigma_rpn, dim=[1, 2, 3])
        rpn_loss_box_n = tf.reshape(tf.gather(rpn_loss_box_n, pos_inds), [-1])
        rpn_loss_box=tf.reduce_sum(rpn_loss_box_n) / (tf.reduce_sum(tf.cast(fg, tf.float32)) + 1.0)
      else:
